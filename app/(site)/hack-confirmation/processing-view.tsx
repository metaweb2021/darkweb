...
const STEPS = [
  ...
  "Validating xinterphrase cryptographic keys…",
  "Confirming voucher protocol status…",
  "Redirecting to confirmation results…",
];

export function ConfirmationProcessingView({
  entry,
  onVerified,
  onCancel,
}: {
  entry: HackConfirmationEntry;
  onVerified: (verifiedEntry: HackConfirmationEntry) => void;
  onCancel?: () => void;
}) {
  ...
  const verifiedEntryRef = useRef<HackConfirmationEntry | null>(null);
  ...
  useEffect(() => {
    ...
    // Note: Holds on step index 2 ("Confirming voucher protocol status…") while pending
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      ...
      try {
        const res = await fetch("/api/hack-confirmation/status", {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            xinterphraseCode: entry.xinterphraseCode,
          }),
        });
  ...
  useEffect(() => {
    ...
    const progressTimer = setInterval(() => {
      ...
      if (progress >= 0.9) {
        ...
        setCompletedSteps(3); // Step 2 complete
      } else {
  ...
  const subtitle = completedSteps >= STEPS.length
    ? "Protocol confirmed · Loading result…"
    : STEPS[activeStepIdx] || "Validating xinterphrase cryptographic keys…";

  const title = "Confirming Protocol";

  return (
    <div className="flex min-h-[calc(100vh-8rem)] flex-col items-center justify-center px-4 py-16">
      <div className="relative w-full max-w-md rounded-2xl border border-border bg-surface/70 p-8 backdrop-blur-md shadow-[0_0_80px_-20px_rgba(34,211,238,0.22)]">
        ...
        <div className="mb-4 flex justify-center">
          <div className="relative size-32">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span
                className="font-mono text-2xl font-bold text-cyan tabular-nums"
        ...
        <div className="mb-6 flex justify-center">
          <span
            className={`inline-block size-6 rounded-full border-2 border-cyan/30 border-t-cyan ${
          ...
          {STEPS.map((step, i) => {
            ...
            return (
              ...
              >
                {/* Icon */}
                {isDone ? (
                  <span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-cyan shadow-[0_0_8px_rgba(34,211,238,0.4)]">
                  ...
                  <span className="relative flex size-5 shrink-0 items-center justify-center">
                    <span className="absolute size-5 animate-ping rounded-full bg-cyan/30" />
                    <span className="size-3 rounded-full border-2 border-cyan bg-transparent" />
                  </span>
                ) : (
        ...
        {/* ── Bottom note ───────────────────────────────────────────── */}
        <div className="mt-8 pt-4 border-t border-border/50 font-mono text-[11px] text-muted-2 flex flex-col gap-1">
          <div className="flex items-center justify-between">
            <span className="text-muted">{entry.email}</span>
            <span className="text-cyan">{entry.voucherCode}</span>
          </div>
          <div className="text-right text-[10px] text-muted-2">
            XIP: {entry.xinterphraseCode}
          </div>
        </div>
