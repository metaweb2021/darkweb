...
export function VoucherConfirmationView({ initialEntries }: { initialEntries: VoucherConfirmationEntry[] }) {
  const [entries, setEntries] = useState(initialEntries);
  const [editingTarget, setEditingTarget] = useState<VoucherConfirmationEntry | null>(null);
  const [deletingTarget, setDeletingTarget] = useState<VoucherConfirmationEntry | null>(null);
  ...
  return (
    <>
      <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-3">
        {[
          { label: "Total Vouchers", value: entries.length, color: "text-foreground" },
      ...
      <div className="rounded-xl border border-border bg-surface/60 overflow-hidden">
        <div className="border-b border-border px-5 py-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-surface/40">
          <div className="flex items-center gap-2">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="size-4 text-amber-500"><path strokeLinecap="round" strokeLinejoin="round" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" /></svg>
            <span className="font-mono text-xs uppercase tracking-widest text-muted">Voucher Confirmation Directory</span>
            ...
            <div className="flex rounded-lg border border-border bg-surface-2 p-0.5">
              {(["all", "pending", "verified"] as const).map((tab) => (
                <button
                  ...
                  className={`rounded-md px-2.5 py-1 font-mono text-[11px] uppercase tracking-wider transition-colors ${statusFilter === tab ? "bg-surface text-foreground font-semibold shadow-sm" : "text-muted hover:text-foreground"
                    }`}
                >{tab}</button>
              ))}
            </div>
        ...
        {filteredEntries.length === 0 ? (
          <div className="py-16 text-center font-mono text-sm text-muted">
            {search || statusFilter !== "all" ? <p>No Voucher submissions match your search criteria.</p> : <p>No Voucher entries yet.</p>}
          </div>
        ) : (
          <div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-border bg-surface/20">
                    {["Status", "Email", "Voucher Code", "Type", "Value", "Email Sent", "Submitted", "Actions"].map((h) => (
                ...
                <tbody>
                  {paginatedEntries.map((e) => (
                    <VoucherRow key={e.id} entry={e} onEdit={(entry) => setEditingTarget(entry)} onDelete={setDeletingTarget} />
                  ))}
                </tbody>
              </table>
            </div>
              ...
              {paginatedEntries.map((e) => (
                <div key={e.id} className="p-4 flex flex-col gap-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <span className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 font-mono text-[9px] uppercase tracking-wider mb-2 ${e.status === "pending" ? "border-amber-400/40 bg-amber-400/10 text-amber-400" : "border-amber-500/40 bg-amber-500/10 text-amber-500"
                        }`}>
                        {e.status}
                      </span>
...
function VoucherRow({ entry, onEdit, onDelete }: { entry: VoucherConfirmationEntry; onEdit: (entry: VoucherConfirmationEntry) => void; onDelete: (entry: VoucherConfirmationEntry) => void; }) {
  ...
  return (
    <tr className={`border-b border-border transition-colors hover:bg-surface-2/50 ${isPending ? "bg-amber-400/[0.02]" : ""}`}>
      ...
      <td className="px-4 py-3 font-mono text-xs text-muted whitespace-nowrap">{entry.voucherType}</td>
      <td className="px-4 py-3 font-mono text-xs text-muted whitespace-nowrap">{entry.voucherValue}</td>
...
function VoucherEditModal({ entry, onClose, onSaved, onDeleteRequest }: { entry: VoucherConfirmationEntry; onClose: () => void; onSaved: (updated: VoucherConfirmationEntry) => void; onDeleteRequest: (entry: VoucherConfirmationEntry) => void; }) {
  ...
  const [state, formAction, pending] = useActionState(adminUpdateVoucherConfirmation, INITIAL_VOUCHER_STATE);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl border border-border bg-surface/95 p-6 shadow-[0_0_80px_-20px_rgba(245,158,11,0.25)] backdrop-blur-md">
        <div className="mb-6 flex items-start justify-between">
          <div><p className="font-mono text-xs text-muted-2">Voucher Conf. ID: {entry.id}</p><h3 className="text-base font-semibold text-foreground mt-0.5">{entry.email}</h3></div>
        ...
        <form action={formAction} className="space-y-4">
          ...
          <div className="rounded-xl border border-border bg-surface/50 p-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><p className="font-mono text-[11px] text-muted mb-1">Voucher Type</p><p className="font-mono text-xs text-foreground uppercase">{entry.voucherType}</p></div>
              <div className="col-span-2"><p className="font-mono text-[11px] text-muted mb-1">Voucher Value</p><p className="font-mono text-xs text-foreground uppercase">{entry.voucherValue}</p></div>
            </div>
          </div>
        ...
        <VoucherSendEmailButton entry={entry} onEmailSent={onSaved} />
      </div>
    </div>
  );
}

function VoucherDeleteModal({ entry, onClose, onDeleted }: { entry: VoucherConfirmationEntry; onClose: () => void; onDeleted: (id: string) => void; }) {
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const res = await adminDeleteVoucherConfirmation(entry.id);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl border border-red-500/30 bg-surface/95 p-6 shadow-[0_0_60px_-15px_rgba(239,68,68,0.25)] backdrop-blur-md">
        <h3 className="text-base font-semibold text-foreground">Delete Voucher Confirmation</h3>
...
function VoucherSendEmailButton({ entry, onEmailSent }: { entry: VoucherConfirmationEntry; onEmailSent: (updated: VoucherConfirmationEntry) => void; }) {
  ...
  const handleSend = () => {
    ...
    startTransition(async () => {
      const res = await sendVoucherConfirmationEmailAction(entry.id);
